 ...
 */
const RACE_ATTRIBUTE_BONUSES = {
    "Human": { str: 1, dex: 1, con: 1, int: 1, wis: 1, cha: 1 }, // +1 all
    "Elf": { dex: 2 },
    "Dwarf": { con: 2 },
    "Halfling": { dex: 2, luck: 1 },
 ...
 */
export function generateDefaultAttributes(characterClass, race) {
    ...
    // Apply class bonuses
    if (classBonuses.str)
        attrs.str += classBonuses.str;
    if (classBonuses.dex)
        attrs.dex += classBonuses.dex;
    if (classBonuses.con)
        attrs.con += classBonuses.con;
    if (classBonuses.int)
        attrs.int += classBonuses.int;
    if (classBonuses.wis)
        attrs.wis += classBonuses.wis;
    if (classBonuses.cha)
        attrs.cha += classBonuses.cha;
    // Apply race bonuses
    if (raceBonuses.str)
        attrs.str += raceBonuses.str;
    if (raceBonuses.dex)
        attrs.dex += raceBonuses.dex;
    if (raceBonuses.con)
        attrs.con += raceBonuses.con;
    if (raceBonuses.int)
        attrs.int += raceBonuses.int;
    if (raceBonuses.wis)
        attrs.wis += raceBonuses.wis;
    if (raceBonuses.cha)
        attrs.cha += raceBonuses.cha;
 ...
 */
export function generateDefaultCharacterName(characterClass, race) {
    ...
    if (raceName.includes("elf"))
        suffix = "-star";
    else if (raceName.includes("dwarf"))
        suffix = "-stone";
    else if (raceName.includes("human"))
        suffix = "";
    else
        suffix = "";
 ...
 */
export function generateDefaultInventory(characterClass) {
    const basePotions = [
        { name: "Health Potion", type: "potion", description: "Restores 10 HP", weight: 0.5 }
    ];
    const classGear = {
        "Barbarian": [
