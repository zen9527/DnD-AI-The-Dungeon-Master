...
export class WebSocketManager implements ManagerApi {
  ...
  private handleClose(ws: WebSocket, connectionId: string): void {
    ...
    log.info(`[WS] Client disconnected (${connectionId})`);
  ...
  private handleMessage(ws: WebSocket, data: Buffer): void {
    ...
    try {
      ...
      log.error(`[WS] handleMessage error for "${rawStr}":`, error instanceof Error ? error.message : "unknown");
  ...
  private routeMessage(ws: WebSocket, message: WebSocketMessage): void {
    ...
    if (!this.llmRateLimiter.tryConsume(client.id, message.type)) {
      ...
      log.warn(`[WS] Rate limited ${message.type} from ${client.id}`);
    ...
    Promise.resolve(handler(ctx)).catch(error => {
      log.error(`[WS] Handler for ${message.type} failed:`, error instanceof Error ? error.message : error);
    });
  }
  ...
  startTimerBroadcast(gameId: string): void {
    ...
    const interval = setInterval(() => {
      ...
      if (engine.timerExpired) {
        log.info(`[Timer] ${currentPlayer.characterName} ran out of time — advancing the turn`);
